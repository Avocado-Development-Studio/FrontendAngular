...
Run `http-server -o` for a dev server. Navigate to `http://127.0.0.1:8080/`. The application will not reload if you change any of the source files, because it not `ng serve`. Don't even try to use `ng serve` because it's a ready-made project. If you want to use `ng serve` for development, then you need to download and replace the files in your project it the `dist/project-name` directory
